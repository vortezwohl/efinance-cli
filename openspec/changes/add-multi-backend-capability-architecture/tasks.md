...
- [x] 6.6 在前述能力稳定后，再迁移 `equity.price.live`，并补充性能与默认限流验证。

## 7. Provider 扩展命令

- [x] 7.1 设计 provider-specific 扩展命令目录结构与帮助展示规则。
- [x] 7.2 为 `akshare` 增加一组示范性扩展命令组，并接入统一执行骨架。
- [x] 7.3 为扩展命令补充运行时约束校验，确保错误 provider 下无法调用。
- [x] 7.4 为未来 `yfinance` provider 预留扩展命令挂载点与可选依赖接入说明。
...
- [x] 8.5 在首批共享能力全部稳定后，清理旧的 `efinance` 函数驱动注册路径和仅为旧模型服务的兼容代码。
